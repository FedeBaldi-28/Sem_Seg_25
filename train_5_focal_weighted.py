...
criterion = FocalLoss(class_weights=weights_tensor, ignore_index=255, gamma=1.0)
...
if __name__ == '__main__':
    ...
    for epoch in range(start_epoch, EPOCHS + 1):
        ...
        if mean_iou > best_miou:
            best_miou = mean_iou
            torch.save(model.state_dict(), 'best_model_5_focal_weighted.pth')
            print(f"Nuova best mIoU: {best_miou:.2f}% → modello salvato!")

        print(f"Epoch {epoch} completato! Best mIoU finora: {best_miou:.2f}%\n\n")

    torch.save(model.state_dict(), f'final_model_epoch_5_focal_weighted{EPOCHS}.pth')
    print(f"📦 Training finito: modello finale salvato come final_model_epoch_5_focal_weighted{EPOCHS}.pth")
