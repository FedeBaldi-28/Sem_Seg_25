...
if __name__ == '__main__':
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if mean_iou > best_miou:
            best_miou = mean_iou
            torch.save(model.state_dict(), 'best_model_5_focal_no_weighted.pth')
            print(f"New best mIoU: {best_miou:.2f}% → modello salvato!")

        print(f"Epoch {epoch} completated! Best mIoU so far: {best_miou:.2f}%\n\n")

    torch.save(model.state_dict(), f'final_model_epoch_5_focal_no_weighted{EPOCHS}.pth')
    print(f"Training completed: final model saved as final_model_epoch_5_focal_no_weighted{EPOCHS}.pth")
