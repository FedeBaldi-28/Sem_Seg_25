...
train_joint_transform = JointTransform(
    ...
    augment=True,
    strategy='jitter'  # oppure 'flip', 'jitter', o 'none'
)
...
def train(model, train_loader, optimizer, criterion, device, num_classes, epoch):
    ...
    for batch_idx, (imgs_s, labels_s) in enumerate(tqdm(train_loader_gta)):
        imgs_t = next(iter_t)
        ...
        with amp.autocast():
            ...
            if isinstance(source_outputs, tuple):
                src_cx1 = source_outputs[1]
                src_cx2 = source_outputs[2]
...
if __name__ == '__main__':
    ...
    torch.save(model.state_dict(), f'final_model_epoch_4{EPOCHS}.pth')
    print(f"📦 Training finito: modello finale salvato come final_model_epoch_4{EPOCHS}.pth")
