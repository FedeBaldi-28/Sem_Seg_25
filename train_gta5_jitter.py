...
if __name__ == '__main__':
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if mean_iou > best_miou:
            best_miou = mean_iou
            torch.save(model.state_dict(), 'best_model_3b_jitter.pth')
            print(f"Nuova best accuracy: {best_miou:.2f}% → modello salvato!")

        print(f"Epoch {epoch} completato! Best accuracy finora: {best_miou:.2f}%\n\n")

    torch.save(model.state_dict(), f'final_model_epoch_3b_jitter{EPOCHS}.pth')
    print(f"📦 Training finito: modello finale salvato come final_model_epoch_3b_jitter{EPOCHS}.pth")