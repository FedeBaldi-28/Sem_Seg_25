...
train_joint_transform = JointTransform(
    ...
    augment=True,
    strategy='flip-jitter-blur'  # oppure 'flip', 'jitter', o 'none'
)
...
if __name__ == '__main__':
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if mean_iou > best_miou:
            best_miou = mean_iou
            torch.save(model.state_dict(), 'best_model_3b_fl_jit_bl.pth')
            print(f"Nuova best accuracy: {best_miou:.2f}% → modello salvato!")

        print(f"Epoch {epoch} completato! Best accuracy finora: {best_miou:.2f}%\n\n")

    torch.save(model.state_dict(), f'final_model_epoch_3b_fl_jit_bl{EPOCHS}.pth')
    print(f"📦 Training finito: modello finale salvato come final_model_epoch_3b_fl_jit_bl{EPOCHS}.pth")