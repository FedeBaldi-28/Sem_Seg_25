...
#################### LOSS & OPTIMIZER ####################
criterion = DiceLoss(ignore_index=255, smooth=1.0)
...
if __name__ == '__main__':
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if mean_iou > best_miou:
            best_miou = mean_iou
            torch.save(model.state_dict(), 'best_model_5_dice_no_weighted.pth')
            print(f"Nuova best mIoU: {best_miou:.2f}% → model saved!")

        print(f"Epoch {epoch} completed! Best mIoU so far: {best_miou:.2f}%\n\n")

    torch.save(model.state_dict(), f'final_model_epoch_5_dice_no_weighted{EPOCHS}.pth')
    print(f"Training completed: final model saved as final_model_epoch_5_dice_no_weighted{EPOCHS}.pth")
